# Load Libraries
pacman::p_load(tidyverse, ggforce, forcats, geomtextpath, ggtext, showtext)
...
plt_df <- pers_art |>
  ...
  mutate(
    ...
    radius = (year - min(year)) * 1.2 + 1,  # Adjust radius for each year to add space between rings
...
rank_cols <- c("Winner" = "#FF8000", "Semifinalist" = "#006BFF", "Nominee" = "#D9EAFD")

# Customized arched labels
arc_lbl <- plt_df |>
  distinct(name) |>
  rowwise() |>
  mutate(
    x = list(seq(-17, 17, length.out = 2000)),
    y = list(15 + sqrt(1000 - x^2))
  ) |>
  unnest(cols = c(x, y))

# Plot
ggplot(plt_df) +
  geom_circle(aes(x0 = 0, y0 = 0, r = radius, color = rank_fct), size = 2, fill = NA) + # concentric rings
  geom_circle(aes(x0 = 0, y0 = 0, r = radius), size = 0.5, color = "#3B1C32", fill = NA) +
  ...
  geom_text(
    ...
    aes(x = radius-32, y = 0, label = "2018 Nominee"), size = 6.5, color = "#87A2FB", hjust = 0.5, vjust = 1, family = "Averia Serif Libre"
  ) +
  geom_text(
    data = plt_df |> 
      filter(name == "Ashley Christensen" & year == 2016),
    aes(x = radius-32, y = 0, label = "2016, 2017 Semifinalist"), size = 4.5, color = "#7900FF", hjust = 0.5, vjust = 4, family = "Averia Serif Libre"
  ) +
  geom_textpath(
    data = arc_lbl,
    aes(x = x, y = y, label = name, group = name), inherit.aes = FALSE, size = 7, hjust = 0.5, color = "#6D2932", family = "Averia Gruesa Libre") +
  theme_minimal() +
  labs(
  ...
  theme(
    ...
    plot.title = element_markdown(size = 25, face = "bold", family = "Averia Serif Libre", hjust = 0.5, margin = margin(t=15,b=10)),
    plot.subtitle = element_text(size = 22, family = "Averia Sans Libre", color = "#35374B", margin = margin(t=5,b=15)),
    plot.margin = margin(l=10, r=10, t=15, b=5),
    plot.caption = element_text(family = "Rosario", color = "#091057", size = 18, hjust = 1, margin = margin(t=10, b=10))
  )

# Save plot
ggsave("Chef_rings.png", width = 20, height = 19, bg = "#E7FBB4")
